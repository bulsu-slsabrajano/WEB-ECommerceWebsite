...
async function loadCheckout() {
    ...
    // ✅ Read selected IDs from URL — avoids localStorage being blocked by browsers
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', (e) => {
            ...
            // ✅ Pass IDs via URL — no localStorage needed
    ...
    if (confirmOrderBtn) {
        confirmOrderBtn.addEventListener('click', async () => {
            // Get the selected IDs from the URL
            const params = new URLSearchParams(window.location.search);
            ...
            try {
                const response = await fetch('../../db/action/toplaceorder.php', {
                    ...
                    // ✅ Send both total AND the specific item IDs to delete
                    body: JSON.stringify({
